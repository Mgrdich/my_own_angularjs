...
- [x] **Slice 3: `$ControllerProvider` + `ngModule` Registration**
  - [x] Create `src/controller/controller-provider.ts` exporting `$ControllerProvider` (class) per technical-considerations §2.3. Constructor takes `['$provide', $provide]` (mirrors `$FilterProvider`); the provider closes over `$$registry: Map<string, ControllerInvokable>`. **[Agent: typescript-framework]**
  - [x] Implement `$ControllerProvider.register` overloads:
  ...
  - [x] Implement `$ControllerProvider.has(name: string): boolean` → `$$registry.has(name)`. Reachable in both phases (no guard). **[Agent: typescript-framework]**
  - [x] Implement `$ControllerProvider.$get` as `['$injector', ($injector) => createController({ injector: $injector, registry: this.$$registry })]`. The `ReadonlyMap` cast is purely a type seam — the runtime registry is the same `Map` the provider mutates. **[Agent: typescript-framework]**
  - [x] Update `src/controller/index.ts` to re-export `$ControllerProvider`. (Class is exposed for advanced consumers + tests; matches `$FilterProvider` precedent.) **[Agent: typescript-framework]**
  - [x] Register `$controller` on `ngModule` in `src/core/ng-module.ts` per technical-considerations §2.8. Add `.provider<'$controller', ControllerService, $ControllerProvider>('$controller', ['$provide', $ControllerProvider])` BEFORE the existing `'$compile'` registration so the dep graph resolves. **[Agent: typescript-framework]**
  - [x] Extend the `declare module '@di/di-types'` block in `src/core/ng-module.ts` to add `$controller: ControllerService` to the `registry` half and `$controllerProvider: $ControllerProvider` to the `config` half. **[Agent: typescript-framework]**
  - [x] Note: `$ControllerProvider` is intentionally **NOT** re-exported from `src/index.ts` (DI-only providers stay out of the root barrel — same precedent as `$SanitizeProvider`, per technical-considerations §2.9). **[Agent: typescript-framework]**
  - [x] Create `src/controller/__tests__/controller-provider.test.ts` covering provider-standalone behavior (constructed manually with a mock `$provide` exposing a controllable `getPhase()`):
  ...
  - [x] Create `src/controller/__tests__/controller-di.test.ts` covering end-to-end via real `createInjector(['ng', customModule])`:
        - `injector.has('$controller') === true` — config-phase reachability of `$controllerProvider` is verified via the captured-reference round-trip below (the provider resolved cleanly inside the `config()` block). `injector.has('$controllerProvider')` is `false` at run time because the provider-injector cache wipes `'$provide'` and providers live in `providerInstances` (not the run-phase facade's lookup maps) — see the assertion + comment in the test for the precise rationale.
        ...
        - Decorator pattern works: `module.decorator('$controller', ['$delegate', $d => $d])` keeps registration semantics intact (sanity check). The originally-proposed `decorator('$controllerProvider', …)` shape is not supported by the codebase (providers live in `providerInstances`, not the four producer maps the decorator-validation pass at `src/di/injector.ts:684-693` consults); the documented seam is the SERVICE-side decorator.
        - `src/__tests__/cross-spec-smoke.test.ts` does NOT exist — bullet intentionally skipped per the "if it exists, otherwise skip" qualifier in the original plan. Confirmed via `ls src/__tests__/` (directory absent). **[Agent: vitest-testing]**
  - [x] Run `pnpm lint`, `pnpm typecheck`, `pnpm test`, `pnpm build`. New public observable: `injector.has('$controller') === true`. All prior tests pass unchanged. Compiler integration NOT yet active — directives that declare `controller` are still ignored at link time (Slice 4 wires that). **[Agent: rollup-build]**

- [x] **Slice 4: `$compile` Per-Element Controller Seam + Registration-Phase `controllerAs` Validation**
  - [x] Extend `normalizeDirective` in `src/compiler/compile-provider.ts` per technical-considerations §2.7. Insert a `normalizeController` block AFTER the existing `normalizeTransclude` / template normalization and BEFORE the priority assignment. Validate:
  ...
  - [x] Modify the per-element linker in `src/compiler/compile.ts` per technical-considerations §2.7. **Insertion point:** between line 637 (end of `$transclude` build / `setElementScope` at line 645) and line 680 (start of pre-link loop), AFTER `bindAttrsToScope` at line 679. For each `directive` in `effectiveDirectives` where `directive.controller != null`:
        ...
        4. Continue to the next directive on throw (matches the linker's existing per-directive isolation pattern at lines 684-688). Mirrored at the post-`templateUrl`-install path inside `buildPostTemplateLinker` so directives whose controllers were deferred behind an async template still fire at the same point in the post-template link cycle. The shared logic is extracted into a small `runControllerSeam(directives, scope, element, attrs, $transclude?)` helper closed over `$controller` + `exceptionHandler`. **[Agent: typescript-framework]**
  - [x] Create `src/controller/__tests__/controller-compile.test.ts` covering FS §2.4 + cross-cutting integration (uses real `createInjector(['ng', module])` + real `$compile` + jsdom `Element`):
