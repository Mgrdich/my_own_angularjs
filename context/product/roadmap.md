...
- [ ] **Directives & DOM Compilation**
  ...
  - [ ] **Multi-element / ranged directives:** Support the `directiveName-start` / `directiveName-end` suffix pair (`ng-repeat-start`/`-end`, `ng-if-start`/`-end`, etc.) so a directive can span a range of sibling elements. Adds a `multiElement` directive flag plus a start/end grouping pass in the compiler walker. Full AngularJS-parity built-in set (`ng-repeat`, `ng-if`, `ng-show`, `ng-hide`, `ng-switch-when`, `ng-switch-default`, `ng-class`) plus custom-directive opt-in. _(spec 033.)_
