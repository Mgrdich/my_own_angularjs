...
- [ ] **Directives & DOM Compilation**
  ...
  - [ ] **Module DSL `.directive` / `.controller`:** Expose `.directive(name, fn)` and `.controller(name, fn)` on `createModule(...)` — both with the single-name and bulk object-map forms — as thin wrappers over `$compileProvider.directive` and `$controllerProvider.register`. ng-module parity, shared registries, no duplicated state. _(spec 021 — `.component` carved out to the Components item below, since it depends on isolate scope / `bindToController` / lifecycle hooks that don't exist yet.)_
  ...
  - [ ] **`$compileProvider` configuration methods:** Add the config-phase knobs absent today — `aHrefSanitizationTrustedUrlList` / `imgSrcSanitizationTrustedUrlList` (URL safe-list patterns), `debugInfoEnabled` (incl. attaching the debug marker classes + scope retrieval so the toggle is meaningful), `commentDirectivesEnabled`, `cssClassDirectivesEnabled`, `strictComponentBindingsEnabled`. _(spec 034.)_
