...
- [ ] **Directives & DOM Compilation**
  ...
  - [ ] **Structural-directive correctness:** Report a clear error when two structural directives share one element (closes the spec-027 same-element terminal-cutoff gap) and eliminate the spurious caught `$compile` errors emitted on every structural-directive clone re-link (uncovered during spec 031 verification). Natural prerequisite for multi-element directives. _(spec 032.)_
