...
- [ ] **Directives & DOM Compilation**
  ...
  - [ ] **Multi-element / ranged directives:** Support the `directiveName-start` / `directiveName-end` suffix pair (`ng-repeat-start`/`-end`, `ng-if-start`/`-end`, etc.) so a directive can span a range of sibling elements. Adds a `multiElement` directive flag plus a start/end grouping pass in the compiler walker. _(deferred — own spec.)_
  - [ ] **`$compileProvider` configuration methods:** Add the config-phase knobs absent today — `aHrefSanitizationTrustedUrlList` / `imgSrcSanitizationTrustedUrlList` (URL safe-list patterns), `debugInfoEnabled`, `commentDirectivesEnabled`, `cssClassDirectivesEnabled`, `strictComponentBindingsEnabled`. _(deferred — own spec.)_
