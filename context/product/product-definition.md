...
- **HTML Sanitization ($sanitize):** Opt-in companion to `$sce` for scrubbing untrusted HTML. Ships as a separate `ngSanitize` module (not in core `ng`) with an ESM-first `createSanitize` / `sanitize` factory and a `$sanitize` / `$SanitizeProvider` DI shim. Tag + attribute allow-lists and a URL-protocol safe-list strip `<script>`, `javascript:` URIs, inline event handlers, and other dangerous constructs. `$sce.getTrustedHtml` optionally falls back to `$sanitize` when the module is loaded, matching the classic `ng-bind-html` pipeline. Documented escape hatch for swapping in DOMPurify via a `$sanitize` decorator.
...
- **Application Bootstrap:** Typed, ESM-first startup API. `bootstrapInjector(modules, config?)` creates a headless injector for tests / SSR / CLI (no DOM). `bootstrap(element, modules, config?)` wires `$compile(element)($rootScope)` + `$rootScope.$apply()` and returns `{ injector, rootScope, rootElement }` — no hidden global `angular`, no mandatory DOM data attachment. `autoBootstrap(root?)` opts into the classic `ng-app` attribute scanner for migration. Generic over the modules tuple so `injector.get('$sce')` retains the correct return type.
