...
Scope.prototype.$watch = function (watchFn, listenerFn, valueEq) {
  let watcher = {
    ...
    listenerFn:
      ...
      function () {
        /**/
      }, // A listener function which will be called whenever that data changes now from lib reference thingy,
...
Scope.prototype.$digest = function () {
  ...
  if (this.$root.$$applyAsyncId) {
    //calling from digest we will cancel pending and flush immediately
  ...
  do {
    ...
    while (this.$$asyncQueue.length) {
      //first async queue to be consumed then after the digest is over its digest will get working
      try {
    ...
    if ((dirty || this.$$asyncQueue.length) && !ttl--) {
      //if the watch keeps scheduling and eval async
      this.$clearPhase();
...
Scope.prototype.$$digestOnce = function () {
  ...
  this.$$everyScope((scope) => {
    //check the use case of this arrow function
    let newValue, oldValue;
    def.Lo.forEachRight(scope.$$watchers, (watcher) => {
      //so it can keep iterating over the new watchers
      try {
        if (watcher) {
          //is it iterating over an undefined because Lodash forEachRight checks the lenght of the array during the start
...
Scope.prototype.$evalAsync = function (expr) {
  ...
  if (!this.$$phase && !this.$$asyncQueue.length) {
    //second for two evalAsync only work once :)
    setTimeout(() => {
...
Scope.prototype.$watchGroup = function (watchFns, listenerFn) {
  ...
  if (!watchFns.length) {
    ...
    return function () {
      //if this invoked it will prevent the listener of the eval async to work
      shouldCall = false;
    };
  }
...
Scope.prototype.$$everyScope = function (fn) {
  ...
  if (fn(this)) {
    //here it invokes the fn once for current scope
  ...
  let internalWatchFn = function (scope) {
    ...
    if (def.Lo.isObject(newValue)) {
      if (def.Lo.isArrayLike(newValue)) {
        //considers array like if its not a function and has a length
        if (!def.isArray(oldValue)) {
          //if the previous is not array so it is changed
          changeCount++;
          oldValue = [];
        }
        ...
        def.Lo.forEach(newValue, function (newItem, index) {
          ...
          if (!bothNaN && newItem !== oldValue[index]) {
            //both are non not to be considered a change
            changeCount++;
        ...
        def.Lo.forOwn(newValue, function (newItem, key) {
          ...
          if (oldValue.hasOwnProperty(key)) {
            ...
            if (!bothNaN && newItem !== oldValue[key]) {
              //both are non not to be considered a change
              changeCount++;
...
Scope.prototype.$$fireEventsOnScope = function (eventName, listenerArgs) {
  let listeners = this.$$listeners[eventName] || [];
  let i = 0;
