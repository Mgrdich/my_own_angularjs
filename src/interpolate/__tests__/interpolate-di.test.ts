...
describe('$interpolate DI integration — Slice 5', () => {
  ...
  // Spec 012 slice 6 makes `$InterpolateProvider.$get` depend on `$sce`,
  // spec 014 slice 7 adds `$exceptionHandler`, and spec 016 slice 4 adds
  // `$filter` as the third dep — all must be re-registered here or the
  // injector can't build `$interpolate`.
  beforeEach(() => {
    resetRegistry();
    createModule('ng', [])
