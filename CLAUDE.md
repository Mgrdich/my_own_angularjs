...
- `package.json` declares `main`, `module`, `types`, and a subpath `exports` map (`.`, `./core`, `./parser`, `./di`, `./compiler`, `./interpolate`, `./sce`). Each build entry emits `.mjs`, `.cjs`, and `.d.ts` via Rollup.
